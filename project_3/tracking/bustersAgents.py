    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # get the most likely ghost position
        ghost_pos = []
        for i in livingGhostPositionDistributions :
            ghost_pos.append(i.argMax())
        
        #chooses an action that brings Pacman closest to the closest ghost
        low_cost = float('inf')
        action = None
        ...
        for i in legal:
            ...
            for j in ghost_pos:
                cost = self.distancer.getDistance(successorPosition, j)
                if (cost < low_cost):
                    action = i
                    low_cost = cost
        return action
