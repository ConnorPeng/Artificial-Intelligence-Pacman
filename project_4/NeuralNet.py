...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        temp = inActs.copy()
        temp.insert(0, 1)
        weighted_sum = self.getWeightedSum(temp)
    ...
    def sigmoidDeriv(self, value):
        ...
        """
        output = (exp(value))/(pow((exp(value) + 1), 2))
        return output
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        temp.insert(0, 1)
        weighted_sum = self.getWeightedSum(temp)
        output = self.sigmoidDeriv(weighted_sum)
        return output
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i, j in enumerate(self.weights):
            wei = j + (alpha * delta * temp[i])
            w.append(wei)
            totalModification = totalModification + abs(wei - j)
        self.weights = w
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for i in self.layers:
            input = []
            for j in i:
                input.append(j.sigmoidActivation(temp))
            result.append(input)
            temp = input
        return result
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    while (iteration < maxItr) and (weightMod > weightChangeThreshold):
        weightMod = nnet.backPropLearning(examplesTrain, alpha)
    ...
    for i, j in examplesTest:
        check = True
        output = nnet.feedForward(i)[-1]
        n = len(output)
        for k in range(n):
            if round(output[k]) != j[k]:
                check = False
        if check:
            testCorrect += 1
        else:
